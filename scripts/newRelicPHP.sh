...
if [ -z "${ispkg}" -a "${arch}" = "x64" -a "${ostype}" != "freebsd" ]; then
...
for pmv in "20060613" "20090626" "20100525" "20121212" "20131226"; do
...
add_to_path /usr/local/php-5.2/bin
...
#
  ...
  if [ "${pi_inidir_cli}" = "/etc/php5/cli/conf.d" -a -z "${pi_inidir_dso}" ]; then
    ...
    if [ -d "/etc/php5/apache2/conf.d" ]; then
      pi_inidir_dso="/etc/php5/apache2/conf.d"
    fi

    if [ -d "/etc/php5/fpm/conf.d" ]; then
      pi_inidir_dso="/etc/php5/fpm/conf.d"
    fi
  fi
...
In order for the ${bold}New Relic${rmso} agent to function correctly it
requires a license key. Please enter that key now. If you do not have
your license key handy you can add it to your INI file(s) later. Please
be aware that the license key you specify here will be installed in the
default INI files, and will be the key used for any virtual host or
directory / application that does not override the key on a per-host or
per-directory basis. This is only relevant if you run a multi-tenant
site. Please contact ${bold}http://support.newrelic.com${rmso} if you
run such a site and have any questions.

If you are upgrading from a previous version please leave this blank.
Please also note that the key you enter here will ${bold}not${rmso}
replace any existing key in your INI file(s). It applies only to newly
created INI files or INI files that have not been modified by this
script before.
...
#
  ...
  if [ -z "${ispkg}" -a -z "${idaemon}" ]; then
    if [ -z "${NR_INSTALL_SILENT}" ]; then
      cat <<EOF
The agents need the ${bold}New Relic${rmso} proxy daemon to be installed
and running. Without this daemon no data will be transmitted to the
${bold}New Relic${rmso} web site. It is highly recommended that you
allow this script to install the daemon into the default location, which
is:

    ${daemonloc}

If you do not have permission to install the daemon to this directory
(you need to be root to have permission) then please select no at the
prompt below. Also please note that as of version 3.0 of the New Relic
PHP agent this daemon does not need to be started automatically by the
system. The daemon is configured from, and started by, the agent. Please
refer to the New Relic for PHP documentation for details.

EOF
      if [ -x "${daemonloc}" ]; then
        cat <<EOF
A file by that name already exists, and should you choose to continue,
it will be replaced by this script.

EOF
      fi

      if getyn "   Install daemon"; then
        idaemon=yes
      else
        idaemon=no
      fi
...
The last step before you will start seeing data in your account is to
restart your web server or servers, or whatever environment hosts the
PHP you wish to monitor. For example, you may need to restart Apache
or nginx, or if you are using php-fpm and FastCGI, you will need to
restart php-fpm. If you installed ${bold}New Relic${rmso} into multiple
locations you may have multiple PHP hosts to restart. If everything is
working properly you should see the following two log files:
...
and point your browser at the test script, and you should see the
${bold}New Relic${rmso} extension and its settings. If you still do not
see those log files or data in your account please contact our support
staff at ${bold}http://support.newrelic.com${rmso} and be sure to
include the following file in your bug report:
